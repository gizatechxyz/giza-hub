---
title: Optimizer
description: "Understanding Giza's capital allocation optimization engine"
---
...
```typescript Max Per Protocol
{
  kind: WalletConstraints.MAX_AMOUNT_PER_PROTOCOL,
  params: { max_amount: "5000000000" }
}
// Cap any single protocol at 5000 USDC
```

```typescript Max Specific Protocol
{
...
// Cap Moonwell specifically at 2000 USDC
```
...
```typescript Min Amount
{
  kind: WalletConstraints.MIN_AMOUNT,
  params: { min_amount: "100000000" }
}
// Don't allocate less than 100 USDC to any protocol
```
...
```typescript
import { GizaAgent, Chain, WalletConstraints } from '@gizatech/agent-sdk';
