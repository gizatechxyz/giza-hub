...
sequenceDiagram
    ...
    App->>SDK: createSmartAccount(origin_wallet)
    SDK->>API: POST /smart-accounts
    API-->>SDK: smartAccountAddress
    SDK-->>App: SmartAccountInfo
    
    Note over App: User deposits to smart account
    
    App->>SDK: activate(wallet, protocols, tx_hash)
...
<Card title="API Reference" icon="code" href="/api-reference/agent/activate">
...
<Card title="API Reference" icon="code" href="/api-reference/agent/get-fees">
  See getFees() for details
</Card>
