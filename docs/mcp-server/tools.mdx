...
The MCP server exposes **23 tools** across 8 groups. Tools that operate on an agent require a **connected wallet** -- call `connect_wallet` first, or the tool returns an error prompting the LLM to connect one.
