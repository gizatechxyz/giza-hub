...
The server reads `GIZA_API_KEY`, `GIZA_PARTNER_NAME`, `GIZA_API_URL`, `CHAIN_ID`, `TRANSPORT`, and `PORT` from the environment. See [Environment Variables](/mcp-server/quickstart#environment-variables) for the full list.
