...
`@gizatech/mcp-server` exposes the Giza Agent SDK as **tools** that any LLM can call through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/). Instead of writing TypeScript to interact with the SDK, an LLM like Claude or GPT calls tools directly -- creating smart accounts, activating agents, checking portfolios, and running optimizations through natural language.
...
| [Optimizer](/mcp-server/tools#optimizer) | `optimize` | No |
...
<CardGroup cols={2}>
  ...
  <Card title="Tools Reference" icon="wrench" href="/mcp-server/tools">
    Complete reference for all 22 tools
  </Card>
  ...
  <Card title="Yield Skills" icon="wand-magic-sparkles" href="/skills#yield-skills">
    Pre-built Claude Code skills that guide end users through onboarding, portfolio monitoring, and fund management via MCP tools
  </Card>
</CardGroup>
