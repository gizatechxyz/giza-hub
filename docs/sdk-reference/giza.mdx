...
The optimizer provides capital allocation recommendations and simulations. See the [Optimizer reference](/sdk-reference/optimizer) for full documentation.
...
```typescript
async simulate(options: SimulateOptions): Promise<SimulationResponse>
```

Simulates allocation results for a given balance without executing any transactions.
...
console.log('Simulated APR:', sim.weighted_apr_final);
```
